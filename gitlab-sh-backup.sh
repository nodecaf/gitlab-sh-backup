...
for GIT_REPO in $( echo $ALL_PROJECTS | jq | grep -i ${GITLAB_GROUP} | sed -e 's/.*://g' -e 's/"//g' )
  do
    #make subdir and clone
    mkdir -p `dirname ${GIT_REPO}`
    cd `dirname ${GIT_REPO}`
    git clone https://${GITLAB_USER}:${GITLAB_TOKEN}@${GITLAB_SERVER}/${GIT_REPO}
    cd `basename -s .git ${GIT_REPO}`
    #Pull down all branches
    for branch in $(git branch --all | grep '^\s*remotes' | egrep --invert-match '(HEAD)'); 
    do
