...
RESPONSE=$( \
	...
	"${GITLAB_SERVER}/api/v4/groups/${GITLAB_GROUP}/projects" \
...
if [[ "$LAST_PAGE" -gt "1" ]]; then
  ...
  for PAGE in $( seq 2 "$LAST_PAGE" ); do
    ...
    RESPONSE=$( \
                ...
                "${GITLAB_SERVER}/api/v4/groups/${GITLAB_GROUP}/projects?${PROJECT_ATTRIBUTES}&page=${PAGE}" \
                | jq '.[].ssh_url_to_repo' \
              ) || { echo "ERROR: Could not retrieve existing project."; exit $?; }
    ALL_PROJECTS+=$'\n'"$RESPONSE"
  done
fi
echo $ALL_PROJECTS
